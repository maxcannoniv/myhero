...
- [x] **3.16 Asset gallery** — Read-only image reference in the admin portal. Shows all images in the system (character profiles + cutouts, faction banners, place backgrounds) with asset name, URL, and Copy URL button. Thumbnails removed: loading full-resolution PNGs (2–3 MB each) as thumbnails was exhausting browser tab memory and crashing Chrome. Images must be compressed to <200 KB before committing — see IMAGE SIZE WARNING in CLAUDE.md Asset System section.
